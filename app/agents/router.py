    ...
    async def run(self, payload: dict[str, Any]) -> AsyncGenerator[str, None]:
        ...
        async with self.tracer.span(
            ...
            new_state = self.store.state.model_copy(
                update={"status": "routing", "current_agent": "router"}
            )
            patch = self.store.apply(new_state)
            yield self.emitter.state_delta(patch)

            # Hand off to research agent, passing history and tracer along.
            ...
            async for event in specialist.run({**payload, "_trace_id": run_id, "_parent_id": root_span.span_id}):
                yield event

            # Finalise.
            final_state = self.store.state.model_copy(
